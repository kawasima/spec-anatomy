...
`status: deprecated` とした spec-model に対応する API エンドポイントや旧入力形式は、削除してよい時期を判断するために流入量を監視します。spec-model の `retire_when` の `metric` フィールドから参照するメトリクス名を、ここで規約化します。

### 規約

- 廃止予定のエンドポイント・旧入力形式・旧 JSON フィールドには、流入カウントメトリクスを実装する
- メトリクス名は `ingress.<endpoint>.<variant>.count` のような形に揃える (例: `ingress.orders.legacy_format.count`)
- 該当する spec-model の Front matter `retire_when` で同じメトリクス名を参照する
- 流入が `expect: 0` の状態で `window` (例: 30d) 続いたら削除候補

```yaml
# spec-model/orders.md の Front matter
status: deprecated
retire_when:
...
# Shell 側の実装イメージ
POST /api/v1/orders
  リクエストボディに "shipping_address" (旧形式) が来たとき:
    metrics.increment("ingress.orders.legacy_format.count")
    互換 decoder で現在の FulfillmentPlan に変換
```

メトリクスは観測コード自体が捨てやすい形で実装します。エンドポイントごとのディレクトリに切り、メトリクス名にエンドポイント識別子を含め、grep で残骸を見つけられるようにしておくと、削除フェーズで観測コードだけ取り残されるのを防げます。
