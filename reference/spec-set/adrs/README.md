...
実装方針の判断を記録する場所です。永続モデル選定や、Spec Set の運用判断、Shell 側の技術選定などを ADR として残します。
...
- 永続表現の `representation_version` バンプ判断
...
ADR の中心は「実装への含意」セクションです。エージェントに守らせる制約を具体的に書きます。
...
`payment.isActivated` のような実装詳細を露出する設計は避け、業務上意味のある振る舞いを公開する制約を残します。Always-valid ドメインモデルの考え方とも整合します。
...
## representation_version バンプ用 ADR テンプレート

`shell/persistence/README.md` の永続表現の版管理に従って `representation_version` をバンプするときは、専用の ADR を残します。仕様モデルの変更と DB レイアウトの変更を結びつけ、旧版を捨てるための条件を ADR 上で追跡できるようにする目的です。

````markdown
# ADR-XXX <集約名> の representation_version を N → N+1 にバンプ
...
- バンプの契機となった spec-model 変更:
  - 例: spec-model/orders.md の data 注文 に Fulfillment 軸の OR 分割を導入
- 旧版 (vN) のレイアウト:
  - 例: shipping_address カラム単独
- 新版 (vN+1) のレイアウト:
  - 例: fulfillment_kind, fulfillment_payload の2カラム

## 決定

- representation_version を N+1 とする
- 既存行は representation_version=N のまま残し、書き込みは N+1 で行う
- 読み出しは N と N+1 の両方の decoder を持つ

## 実装への含意

- shell/persistence/<table>.md に N+1 の decoder/encoder を追加
- 旧 decoder は spec-model/<file>.md の owns.representation_versions に [N, N+1] と列挙
- spec-model 側は新版に対応する data を主、旧版互換のための分岐を Why で説明

## retire_when (旧版の削除条件)

このバンプに伴って、旧版を捨てる条件を spec-model/<file>.md の retire_when に書く。

```yaml
retire_when:
  - description: 旧版で書かれた行が0件
    sql: "select count(*) from <table> where representation_version = N"
    expect: 0
```
...
- Core: <該当する data / behavior>
- Shell:
  - persistence: <table>.<column> (追加 / 削除)
  - api: 該当エンドポイント

## 検証方法

- 永続モデルテスト: 各 representation_version の decoder が現行ドメインモデルを返すか
- retire_when 評価ジョブの結果を CI で監視

## 結果

（バンプ後に旧版が残っているか、retire_when が満たされた日付を追記）
````

このテンプレートで残す情報のうち最も重要なのは「旧版の削除条件」です。バンプ ADR を書きながら旧版の retire_when を一緒に決めると、新版に切り替えるための仕事と、旧版を捨てるための仕事が同じ意思決定の中で扱われます。

## サンプル

実プロジェクトでの最初の ADR として、永続モデル選定（ADR-001）と認証方式選定（ADR-002）を書くと、後続の ADR の書き方の指針になります。具体例は本リポジトリ同梱のサンプルを参照してください。
