...
この設計判断は、仕様モデル駆動設計（SMDD）の **インサイドアウト開発** と **Core/Shell の二段階生成** の指針に従っています。Traditional SIerの設計書がしばしば置いてきた「画面駆動」「テーブル駆動」「配線プログラミング」を、仕様駆動の名のもとに高速に再生産しないための構造です。

## 構成

```
docs/
├── spec-set/                Spec Set の規約・テンプレート
│   ├── README.md            この文書
│   ├── design.md            設計方針の詳細
...
│   └── adrs/                ADR（実装方針の記録）
│       └── README.md
└── examples/                Spec Set を適用した具体例
    └── business-trip/       出張申請の完全サンプル
        ├── README.md
        ├── spec-model/      Core の例
...
- **Core（spec-model/）** は仕様DSL で書かれます。フレームワーク、DB、UIライブラリへの依存を一切持ちません。エージェントへの入力（**Strict Spec**：本質的複雑さを曖昧さゼロで渡す）として、業務ルールを伝えます。
...
仕様モデル駆動設計（SMDD）の標準的な進め方はゼロから Core を書き始めるインサイドアウトですが、SIer の現場では既存設計書（画面・テーブル・帳票）が先にあるケースが大半です。このSpec Setはその経路にも対応します。
