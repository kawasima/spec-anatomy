...
経済性逆転（仕様モデル更新が実装変更の最短経路になる）は検証ループが回ることが前提で、仕様テストの3軸と組み合わせて運用します。

## なぜ検証ループが必要か

「仕様モデル更新が実装変更の最短経路になる」という経済性逆転は、検証ループが回ることが前提です。
...
`status: deprecated` とした spec-model の Front matter `retire_when` を CI で評価し、削除候補を検出します。仕様自体の Disposability を運用するための層で、`status` フィールドが付くだけで実際の削除トリガーがない、という状態を防ぎます。

#### 規約

- spec-model 全体を走査し、`status: deprecated` のファイルを列挙する
- 各ファイルの `retire_when` の各条件を実行する
  - `sql:` の条件は dev / staging DB に対してクエリを発行し、結果が `expect:` (通常 0) と一致するか確認
  - `metric:` の条件は監視基盤に問い合わせ、`window` 期間で `expect` を満たすか確認
- すべての条件が満たされた spec-model を削除候補として出力する
- 削除候補は PR コメントまたは別レポートとして可視化し、定期的に片付ける

```text
# 出力イメージ
削除候補:
  - spec-model/orders.md
    status: deprecated
    retire_when すべて満たし済み:
      - "select count(*) from orders where representation_version = 1" → 0 (expect: 0)
      - ingress.orders.legacy_format.count (30d) → 0 (expect: 0)
    関連削除対象 (owns):
      persistence: orders.shipping_address, orders.legacy_billing_id
      api: POST /api/v1/orders (legacy_format)
      representation_versions: [1]
```

削除候補が出たら、`owns` に列挙された Shell 側の構成要素 (テーブルカラム、API エンドポイント、互換 decoder) と一緒に削除します。Piranha 系の codemod による自動削除はこの規約のスコープ外で、検出までを規約化する範囲とします。

#### `retire_when` を書かずに deprecated を付けない

`status: deprecated` だけ付けて `retire_when` を書かない仕様は、削除のトリガーがないまま腐る原因になります。spec-model lint で次を強制します。

- `status: deprecated` のとき `retire_when` が空でない
- `retire_when` の各条件が `sql` または `metric` のいずれかを持つ
- `metric` を持つ条件の `metric` 名が `shell/api/` の規約に従っている (`ingress.<endpoint>.<variant>.count` 等)
...
破壊的変更や制約強化はビッグバンリリースを避けます。Expand and Contract パターンで段階的に適用します。
...
- `retire_when` 評価（deprecated 仕様の削除候補検出）
...
- Sam Newman *Building Microservices*（Expand and Contract、データベースリファクタリング）
