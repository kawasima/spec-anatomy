...
#### 軸候補列挙ステップ (A1-A3) を遡及適用すると

`refactoring-with-agents.md` のステップ4-A1〜A3 を後付けで適用すると、`請求` の OR 分割を採るプロセスは次のように記録できます。

- A1: AI が提示しうる軸候補
  - 枠消費の有無で分ける (枠内 OR 超過込み)
  - 請求周期で分ける (月次 OR 単発)
  - 顧客タイプで分ける (Premium OR Standard)
  - 永続表現で分ける (legacy_billing OR new_billing)
- A2: 評価チェックリストでの判定
  - 枠消費の有無 → **不変条件が異なる** (超過込みのみ超過レートを必須とする) ので Core の OR にする
  - 請求周期 → 構造は同じで behavior の入力で済む。Core に持ち込まない
  - 顧客タイプ → プラン種別で吸収できる。Core に持ち込まない
  - 永続表現 → `representation_version` (`shell/persistence`) で吸収する
- A3: spec-model に Why として残す
  - 採用した軸: 枠消費の有無
  - 採らなかった軸: 上記3つ。理由は spec-model.md の「設計の意図」セクションに記録

この記録を残しておくことで、将来「単発請求が必要になった」「Premium 専用フィールドが必要になった」と要件が変わったときに、過去の判断を更新する形で軸の取り直しを判断できます。本サンプルでは spec-model.md の Front matter と「採用した軸と採らなかった代替軸」セクションにこの記録を反映済み。
