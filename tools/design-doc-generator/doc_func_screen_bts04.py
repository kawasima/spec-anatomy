...
- 経理連携は共通コンポーネント【CC0003：経理連携データ作成】の呼び出しとして、算出した精算額を
  経理I/Fファイル（IBT01）へ出力する手続きとして書く。さらに固定長レイアウト（申請ID16桁・
  社員番号10桁・精算額ゾーン10桁・費用負担区分1桁・最終承認日時14桁）や、精算額が0円でも
  ヘッダ・データ・トレーラを各1件必ず出力するといったファイル都合の詳細を、画面イベント詳細に
  そのまま書き込む。（afterではこのファイル都合の詳細はShellの永続／外部I/Fに隔離され、
  画面の仕様からは切り離される。）
"""
...
WBT0401_DETAIL = [
    ...
    {"G": "社員番号", "K": 17, "N": 10, "Q": "applicant_id。半角英数字10桁。"},
    {"G": "精算額", "K": 27, "N": 10,
     "Q": "(4)で算出した精算額。ゾーン10進10桁、右詰めゼロ埋め（0円の場合も 0000000000 を出力する）。"},
    {"G": "費用負担区分", "K": 37, "N": 1, "Q": "cost_bearing。1桁（1:自社負担 / 2:先方負担）。"},
    {"G": "最終承認日時", "K": 38, "N": 14, "Q": "finalized_at。YYYYMMDDHHMMSS形式の14桁。"},
    {"F": "ヘッダレコードにはレコード種別'H'とファイル作成日時を、トレーラレコードにはレコード種別'T'・"
          "データレコード件数・精算額合計を出力する。"},
    {"E": "(6) 表示処理"},
