...
Core で定義したドメインイベント（書籍4章のイベント・エンティティ）は、Shell のメッセージング契約として外部システムに通知できる形に変換します。
...
書籍4章のイベント・エンティティは「同じ messageId のイベントを2回受信しても1回しか反映されない」という冪等性が業務上重要です。
...
外部からの入力なので、書籍6.1.1のバリデーションが必要です。
...
- 書籍4章（イミュータブルデータモデル、ドメインイベント）、6.1.1節（バリデーションと不変条件）
