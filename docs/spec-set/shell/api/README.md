...
書籍5.5.2 のエラー分類を HTTP ステータスコードに対応させます。
...
Core の `behavior` のシグネチャと、API のリクエスト/レスポンスが一致していることを契約テストで検証します。書籍5.5節の `behavior` の全域性テストを、API レイヤーでも適用します。
...
API のバージョニング（`/api/v1/`、`/api/v2/`）は Shell の関心事です。Core が変わるたびに新バージョンを切るのではなく、Core の変更が破壊的かどうか（書籍5.7.1の3分類）で判断します。

- 拡張（OR の枝を追加） → 既存の API は壊れない。新枝を扱うクライアントだけが対応する
- 制約強化（Optional を必須化） → API のバリデーションが厳格化される。クライアントの修正が必要
- 破壊的変更 → 新バージョンの API を切る、または書籍8.7.2 の Expand and Contract で並行運用する
...
- 書籍5.5.2節（エラー型設計）、6.4節（関連のモデリング）、8.7節（変更を安全に届ける）
