...
Core の `data` を、データベースのテーブル定義・ORM マッピング・関連の永続化パターンに変換する規約です。書籍6章（特に6.2 と 6.4）に従います。
...
書籍6.2 の主旨です。永続モデル == ドメインモデル（Active Record パターン）と扱うと、業務ルールがデータベーススキーマに従属し、テーブル変更が業務ロジックに波及します。

このSpec Setでは、書籍6.2.2 の3つのパターンから業務の性質に合わせて選びます。
...
書籍6.2.1 の通り、データベーススキーマの制約は永続モデルの型として表現します。
...
書籍6.4 の4つの論点（パリティ・ID/実体・取得タイミング・更新方式）の組み合わせから、業務の性質に合わせて1つを選びます。
...
書籍4章のイミュータブルデータモデルを採るときは、永続化も次のように変わります。
...
イベント系列が長くなると、毎回 fold するコストが効いてきます。途中の状態をスナップショットとして保存し、性質4（書籍5.6.3）の「fold(全体) = fold(fold(前半), 後半)」を利用して再開します。
...
- 書籍6.1.2節（トランザクションスクリプトとドメインモデル）、6.2節（永続モデルとのマッピング）、6.2.1〜6.2.3節（永続モデルの実装パターンと性能優先主義）、6.4節（関連のモデリング）、4章（イミュータブルデータモデル）
- Scrapbox「関連のモデリング」
