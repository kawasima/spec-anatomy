...
書籍6.2 の Data Mapper パターンに従い、永続モデル（テーブル定義）とドメインモデル（Core）を別の型として持ちます。Core の状態ごとに data を分けた構造を、テーブルでは status カラムで表現し、Mapper で双方向に変換します。
...
書籍6.4 の関連のモデリングでは、出張申請と予定費用・実績費用の関係は次のように扱います。
