...
- `behavior 最終承認する` ([../../spec-model/business-trip.md](../../spec-model/business-trip.md))

この画面が表示する `data`。

- `data 出張実績` ([../../spec-model/business-trip.md](../../spec-model/business-trip.md))
- `data 最終承認` ([../../spec-model/business-trip.md](../../spec-model/business-trip.md))
...
| ---------------- | ----- | ------------------------------------------------ | -------------------------------- | ---- | ------ | -------- |
| 申請ID           | label | `出張実績.出張予定.id`                           | BT-{yyyy}-{連番4桁}              | −    | −      | 常に表示 |
| 申請者           | label | `出張実績.出張予定.List<出張者>`                 | 氏名（複数人は「、」区切り）     | −    | −      | 常に表示 |
| 出張目的         | label | `出張実績.出張予定.出張目的`                     | そのまま表示                     | −    | −      | 常に表示 |
| 出張期間         | label | `出張実績.出張予定.出張期間`                     | yyyy/MM/dd 〜 yyyy/MM/dd         | −    | −      | 常に表示 |
| 費用負担区分     | label | `出張実績.出張予定.費用負担区分`                 | 「自社負担」または「先方負担」   | −    | −      | 常に表示 |
| 予定費用合計     | label | `sum(出張実績.出張予定.出張予定費用.予定金額)`   | #,##0 円                         | −    | −      | 常に表示 |
...
| 項目名           | 種別  | 派生元                                              | 編集仕様                                    | 必須 | 初期値 | 表示条件 |
| ---------------- | ----- | --------------------------------------------------- | ------------------------------------------- | ---- | ------ | -------- |
| 費目             | label | `出張予定日程.費目` / `出張実績日程.費目`           | 「交通費」「宿泊費」「交際費」              | −    | −      | 各行     |
| 予定金額         | label | `出張予定日程.予定金額`                             | #,##0 円                                    | −    | −      | 各行     |
| 実績金額         | label | `出張実績日程.実績金額`                             | #,##0 円。予定超過の場合は赤字表示          | −    | −      | 各行     |
| 差額             | label | `出張実績日程.実績金額 - 出張予定日程.予定金額`     | #,##0 円（派生計算。超過時は +XXX円と表示） | −    | −      | 各行     |
| 予定費用合計     | label | `sum(出張予定日程.予定金額)`                        | #,##0 円                                    | −    | −      | フッタ行 |
| 実績費用合計     | label | `sum(出張実績日程.実績金額)`                        | #,##0 円                                    | −    | −      | フッタ行 |
| 差額合計         | label | `sum(実績金額) - sum(予定金額)`                     | #,##0 円（派生計算）                        | −    | −      | フッタ行 |

#### 画面項目グループ: 操作領域

| 項目名           | 種別   | 派生元 | 編集仕様                 | 必須 | 初期値 | 表示条件                                      |
| ---------------- | ------ | ------ | ------------------------ | ---- | ------ | --------------------------------------------- |
| 最終承認ボタン   | button | −      | ラベル「最終承認」       | −    | −      | role >= MANAGER かつ 操作ユーザが申請者の上長 |
...
| 項目名       | 種別  | 派生元                                           | 編集仕様             | 必須 | 初期値 | 表示条件 |
| ------------ | ----- | ------------------------------------------------ | -------------------- | ---- | ------ | -------- |
| 出張目的     | label | `出張実績.出張予定.出張目的`                     | 全角20文字超は省略   | −    | −      | 常に表示 |
| 出張期間     | label | `出張実績.出張予定.出張期間`                     | MM/dd 〜 MM/dd       | −    | −      | 常に表示 |
| 実績費用合計 | label | `sum(出張実績日程.実績金額)`                     | #,##0 円             | −    | −      | 常に表示 |
...
| 項目名         | 種別   | 派生元 | 編集仕様             | 必須 | 初期値 | 表示条件                                      |
| -------------- | ------ | ------ | -------------------- | ---- | ------ | --------------------------------------------- |
| 最終承認ボタン | button | −      | 画面下部右に固定配置 | −    | −      | role >= MANAGER かつ 操作ユーザが申請者の上長 |
